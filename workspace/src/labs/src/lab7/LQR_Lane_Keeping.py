...
class image_processing_node():
    ...
    def calc_x_newPixel_to_y_Inertial(self,x_newPixel,y_newPixel):
        ...
        y_Inertial = (-0.0021648768*x_Inertial-0.00033374)*x_newPixel+ (-0.01913458*x_Inertial**3+0.130408397*x_Inertial**2 - 0.234519248288*x_Inertial+0.08843631)
    ...
    def calc_y_newPixel_to_x_Inertial(self,y_newPixel):
        ...
        x_Inertial = 1.9701e-06*y_newPixel**3 + (-0.00064846)*y_newPixel**2 + 0.069623*y_newPixel - 0.6249
    ...
    def compute_uOpt(self,x_ref,y_ref,v_ref):
        try:
            if not(self.stopmoving):
                ...
                Q = np.matrix([[5,0,0],[0,5,0],[0,0,1]])
                R = 0.05*np.matrix([[2,0],[0,1]])
